...
async fn main() {
	...
	let app = Router::new()
		...
		.route("/systems", axum_codec::routing::get(crate::systems::all).into())
		.route("/systems/subdomain/{root_domain}", axum_codec::routing::get(crate::systems::subdomain).into())
		.route("/systems/http", axum_codec::routing::get(crate::systems::http).into())
		.route("/systems/host/{host}", axum_codec::routing::get(crate::systems::host).into())
		.route("/systems{*_subpath}", get(Redirect::temporary("/systems")))
		.route("/volumes", axum_codec::routing::get(crate::volumes::all).into())
		.route("/volumes{*_subpath}", get(Redirect::temporary("/volumes")))
		.route("/hosts", axum_codec::routing::get(crate::hosts::all).into())
